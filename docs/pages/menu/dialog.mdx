...
相比于 `.show()` 打开的，对话框拥有全屏遮罩（`::backdrop`）    
且默认会被 <T tag='kbd' class='☘-kbd'>Esc</T> 键关闭
...
案例使用 和实验性的 [`position:anchor`](https://developer.mozilla.org/en-US/docs/Web/CSS/position-anchor) 实现

你完全可以用 `[popover]:popover-open+.☘-dialog` 或者仅使用 `dialog.☘-dialog`同js事件 模拟出这个功能

全局变量 `--padding-popover` 可以设置弹框内边距，`.☘-dialog.sm` 预设了更小的
