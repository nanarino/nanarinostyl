...
<Grid inline style={{'--indent-radio': '20px'}}>
    <Radio name="type" id="HTTP" disabled>HTTP</Radio>
    <Radio name="type" id="HTTPS" checked disabled dataset={{ 
        primary : "success" 
    }}>HTTPS</Radio>
</Grid>
...
div(style={'--indent-radio': '20px'})
    span.☘-radio
        input#HTTP.☘-radio(type="radio" disabled)
        label.☘-radio-label(for="HTTP" title="HTTP") HTTP
    span.☘-radio(data-primary="success")
        input#HTTPS.☘-radio(type="radio" checked disabled)
