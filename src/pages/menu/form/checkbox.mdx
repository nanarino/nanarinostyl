---
layout: "@layout/main.astro"
title: 多选框
---

import T from "src/components/T.astro";
import Checkbox from "src/components/checkbox.astro";
import Grid from "src/components/grid.astro";
export * from 'src/components/markdown/map.mdx';


# checkbox

<Checkbox id="world" checked style="--checkbox-indent: 2em">world</Checkbox>

这里实现使用了 `path()` 绘制，如果你的浏览器不支持会被降级，样式会有细微区别

- 全局变量 `--checkbox-indent` 改变label文字左内边距，原本是用 `text-indent` 属性实现。但在Safari浏览器中不生效，遂改用 `padding-left` 来实现。默认值是1em（label紧贴input）

```pug
span.☘-checkbox(style="--checkbox-indent: 2em")
    input#hello.☘-input(type="checkbox")
    label.☘-checkbox-label(for="hello" title="world") world
```

注意，checkbox**捆绑**了 `<label>` ，你必须携带它，默认情况下label被点击时也会触发选择或反选

## `disabled`

<Grid inline style={{'--checkbox-indent': '20px'}}>
    <Checkbox id="HTTP" disabled>HTTP</Checkbox>
    <Checkbox id="HTTPS" checked disabled dataset={{ 
        primary : "success" 
    }}>HTTPS</Checkbox>
</Grid>

```pug
div(style={'--checkbox-indent': '20px'})
    span.☘-checkbox
        input#HTTP.☘-input(type="checkbox" disabled)
        label.☘-checkbox-label(for="HTTP" title="HTTP") HTTP
    span.☘-checkbox(data-primary="success")
        input#HTTPS.☘-input(type="checkbox" checked disabled)
        label.☘-checkbox-label(for="HTTPS" title="HTTPS") HTTPS
```

## `data-indeterminate`

<Checkbox
    id="Superposition"
    dataset={{ indeterminate : true }}
    style="--checkbox-indent: 20px"
    onchange="
    this.parentElement.removeAttribute('data-indeterminate')
">叠加态</Checkbox>

```pug
span.☘-checkbox(
    data-indeterminate
    style="--checkbox-indent: 20px"
)
    input#Superposition.☘-input(
        type="checkbox"
        onchange=function(){
            this.parentElement.removeAttribute('data-indeterminate')
        }
    )
    label.☘-checkbox-label(
        for="Superposition"
        title="Superposition"
    ) 叠加态
```

## custom style

- `--color-checkbox` 全局变量覆盖颜色
- `--color-checkbox-disabled` 禁用状态全局变量覆盖颜色

<Checkbox
    id="BlueSuperposition"
    disabled
    dataset={{ indeterminate : true }}
    style={{
        '--color-checkbox': 'var(--blue-6)',
        '--color-checkbox-disabled': 'var(--blue-2)',
        '--checkbox-indent': '20px'
}}>饿死了</Checkbox>

```pug
span.☘-checkbox(data-indeterminate style={
    '--color-checkbox': 'var(--blue-6)',
    '--color-checkbox-disabled': 'var(--blue-2)',
    '--checkbox-indent': '20px'
})
    input#BlueSuperposition.☘-input(type="checkbox" disabled)
    label.☘-checkbox-label(
        for="BlueSuperposition"
        title="BlueSuperposition"
    ) 饿死了
```

### without label

默认情况下label为空即可

<Checkbox
    id="NoLabel"
    style="--checkbox-indent: 1em"
    dataset={{ indeterminate : true }}
    onchange="
    this.parentElement.removeAttribute('data-indeterminate')
"></Checkbox>

```pug
span.☘-checkbox(data-indeterminate)
    input#NoLabel.☘-input(type="checkbox")
    label.☘-checkbox-label(for="NoLabel" title="NoLabel")
```

如果你只是需要label无法点击，只有checkbox能被点击：

```sass
label.☘-checkbox-label
    pointer-events: none
```