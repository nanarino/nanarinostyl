...
{/**
  ...
  * 开发时请忽略这个404错误
  * 使用绝对路径：
  *   ![正确示例](/src/assets/images/example.jpg)
  * 
  */}
...
.☘-image(style={
    ...
    .☘-image-mask(onclick=function(){
        ...
        svg(width="24px" data-icon='interactive-button/outline/heart')
        svg(width="24px" data-icon='interactive-button/fill/heart-fill')
...
`.☘-input-wrapper` 也可以使用局部变量 `--color-input-wrapper` 修改颜色RGB值
